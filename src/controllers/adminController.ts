...
import {
  ...
  syncAllUsers,
  syncAllTags,
} from "../services/typesenseSync.js";
...
/**
 * Get Typesense status and stats
 */
export const getTypesenseStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (!available) {
      return res.json({
        available: false,
        message: "Typesense not configured or unavailable",
      });
    }
    
    // Get document counts from MongoDB for comparison
    const [postsCount, usersCount] = await Promise.all([
      ImagePost.countDocuments({ isArchived: { $ne: true } }),
    ...
    res.json({
      ...
      message: "Typesense is available",
    });
  } catch (error: any) {
...
/**
 * Full sync - rebuild all Typesense indexes
 */
export const typesenseFullSync = async (req: Request, res: Response) => {
  try {
    const available = await isTypesenseAvailable();
    ...
    console.log("🚀 Admin triggered full Typesense sync");
 ...
 * Sync only posts
 */
export const typesenseSyncPosts = async (req: Request, res: Response) => {
  try {
    const available = await isTypesenseAvailable();
    ...
    console.log("🚀 Admin triggered posts sync");
 ...
 * Sync only users
 */
export const typesenseSyncUsers = async (req: Request, res: Response) => {
  try {
    const available = await isTypesenseAvailable();
    ...
    console.log("🚀 Admin triggered users sync");
 ...
 * Sync only tags
 */
export const typesenseSyncTags = async (req: Request, res: Response) => {
  try {
    const available = await isTypesenseAvailable();
    ...
    console.log("🚀 Admin triggered tags sync");
    
    const result = await syncAllTags();
    
    res.json({
      success: true,
      message: "Tags sync completed",
      result,
    });
  } catch (error: any) {
